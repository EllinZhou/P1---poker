...
class Player:
    ...
    def bid(self, pot, bet):
        # TODO: if the player doesn't have enough money to bid, print a warning. Otherwise, add the bet to the pot.
        if self.money < bet: # bet is the amount that the player tries to bid
            print("Player " + self.id + " doesn't have enough money!")
        else:
            pot.add_bet(self.id, bet)
