...
class Hand(object):
    ...
    def score_hands(self, pot_cards):
        ...
        hands_to_score = [FinalHand(hand) for hand in list(
...
class FinalHand(object):
    ...
    def __init__(self, cards):
        ...
        # dictionary with counts of each card
        self.counter = Counter([card.value for card in self.cards])
    ...
    def score_hand(self):
        ...
        if royal_flush:
            ...
            # add highest card in case of a tie
            score += 3 + (high_pair / 100) + (highest_card / 1000)
        elif one_pair:
...
'''Possible tests you may want to implement:
